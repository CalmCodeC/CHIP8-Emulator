...
int main(int argc, char **argv)
{
	if(argc < 2)
	{
		printf("Initialized the Program. \n\n");
		return 1;
	}
	...
	glutInit(&argc, argv); glutInitDisplayMode(GLUT_DOUBLE | GLUT_RGBA);
        glutInitWindowSize(display_width, display_height);glutInitWindowPosition(640, 480);
	glutCreateWindow("Super Storm CHIP-8 Emulator");

	glutDisplayFunc(display);
	glutIdleFunc(display);
    glutReshapeFunc(reshape_window);
	glutKeyboardFunc(keyboardDown); // initialises key.
	glutKeyboardUpFunc(keyboardUp); // Removes the initialised key.

#ifdef DRAWWITHTEXTURE
	setupTexture();
#endif

	glutMainLoop(); // the render loop for textures.
...
void keyboardDown(unsigned char key, int x, int y)
{
	...
	cout<<"Pressed key"<<key; // for debugging purposes 
}
